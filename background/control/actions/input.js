...
export class InputActions extends BaseActionHandler {
    ...
    async clickElement(args = {}) {
        // Support batch format: { elements: [{uid, dblClick}, ...] }
        if (args.elements && Array.isArray(args.elements)) {
            return this.clickElements({ elements: args.elements });
        }
        // Bare array fallback (AI sometimes sends this format)
        if (Array.isArray(args)) {
            return this.clickElements({ elements: args });
        }
        // Original single click
        return this.mouse.clickElement(args);
    }

    async clickElements({ elements }) {
        if (!elements || !Array.isArray(elements)) {
            return "Error: 'elements' array is required for batch click.";
        }
        ...
        for (let i = 0; i < elements.length; i++) {
            ...
            try {
                if (!item || typeof item.uid !== 'string') {
                    results.push(`Skipped invalid item at index ${i}: ${JSON.stringify(item)}`);
                    continue;
                }

                const res = await this.mouse.clickElement({
                    uid: item.uid,
                    dblClick: Boolean(item.dblClick)
                });
                results.push(res);
            } catch (e) {
                results.push(`Failed to click element ${item?.uid ?? 'index_' + i}: ${e.message}`);
            }
        }

        return `Batch click completed:\n${results.join('\n')}`;
    }
